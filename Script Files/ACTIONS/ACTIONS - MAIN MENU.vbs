...
BeginDialog ACTIONS_scripts_main_menu_dialog, 0, 0, 456, 240, "Actions scripts main menu dialog"
  ButtonGroup ButtonPressed
    CancelButton 400, 220, 50, 15
    PushButton 5, 25, 85, 10, "ABAWD Screening Tool", ABAWD_tool
    PushButton 385, 10, 65, 10, "SIR instructions", SIR_instructions_button
    PushButton 5, 40, 50, 10, "BILS updater", BILS_UPDATER_button
    PushButton 5, 55, 50, 10, "Check EDRS", CHECK_EDRS_button
    PushButton 5, 70, 105, 10, "Child Support Disregard FIATer", CS_FIAT_button
    PushButton 5, 85, 75, 10, "Copy panels to Word", COPY_PANELS_TO_WORD_button
    PushButton 5, 100, 55, 10, " FSET sanction", FSET_sanction_button
    PushButton 5, 115, 105, 10, "LTC-Spousal Allocation FIATer", LTC_SPOUSAL_ALLOCATION_FIATER_button
    PushButton 5, 130, 105, 10, "MA-EPD earned income FIATer", MA_EPD_EI_FIAT_button
    PushButton 5, 145, 60, 10, "New job reported", NEW_JOB_REPORTED_button
    PushButton 5, 160, 60, 10, "PA verif request", PA_VERIF_REQUEST_button
    PushButton 5, 175, 70, 10, "Paystubs Received", PAYSTUBS_RECEIVED_button
    PushButton 5, 195, 45, 10, "Send SVES", SEND_SVES_button
    PushButton 5, 210, 55, 10, "Transfer case", TRANSFER_CASE_button
    PushButton 5, 225, 85, 10, "Update worker signature", UPDATE_WORKER_SIGNATURE_button
  Text 100, 25, 270, 10, "--- NEW 10/15!!! A tool to walk through a screening to determine if client is ABAWD."
  Text 60, 40, 215, 10, "--- Updates a BILS panel with reoccurring or actual BILS received."
  Text 60, 55, 185, 10, "--- sends an EDRS request for a HH member on a case."
  Text 85, 85, 180, 10, "--- Copies MAXIS panels to Word en masse for a case."
  Text 115, 115, 175, 10, "--- FIATs a spousal allocation across a budget period."
  Text 115, 130, 295, 10, "--- FIATs MA-EPD earned income (JOBS income) to be even across an entire budget period."
  Text 70, 145, 380, 10, "--- Creates a JOBS panel, CASE/NOTE and TIKL when a new job is reported. Use the DAIL scrubber for new hire DAILs."
  Text 70, 160, 320, 10, "--- Creates a Word document with PA benefit totals for other agencies to determine client benefits."
  Text 80, 175, 370, 20, "--- Enter in paystubs on one dialog, and it puts that information on JOBS (both retrospective and prospective if applicable), as well as the PIC and HC pop-up, and it'll case note the income as well."
  Text 55, 195, 90, 10, "--- Sends a SVES/QURY."
  Text 65, 210, 325, 10, "--- SPEC/XFERs a case, and can send a client memo. For in-agency as well as out-of-county XFERs."
  Text 95, 225, 185, 10, "--- Updates the default worker signature on your scripts."
  Text 120, 70, 325, 10, "--- NEW 09/2015!!! FIATS in the CS disregard for MFIP and DWP as described in CM 17.15.03"
  Text 5, 5, 245, 10, "Action scripts main menu: select the script to run from the choices below."
  Text 70, 100, 365, 10, "--- NEW 07/2015!!! -- Updates the WREG panel, and case notes when imposing or resolving a FSET sanction."
EndDialog
