...
'>>>>>THE NEW DIALOG<<<<<
BeginDialog shelter_form_received_dialog, 0, 0, 276, 415, "Shelter Expenses Dialog"
  ...
  CheckBox 10, 320, 240, 10, "Check here to have the script update ADDR, SHEL, and HEST.", update_MAXIS_check
  EditBox 60, 340, 210, 15, other_notes
  EditBox 65, 360, 205, 15, actions_taken
  EditBox 75, 395, 70, 15, worker_signature
  ButtonGroup ButtonPressed
    OkButton 170, 395, 50, 15
    CancelButton 220, 395, 50, 15
  ...
  Text 10, 345, 45, 10, "Other notes:"
  Text 10, 400, 60, 10, "Worker signature:"
...
'Going through and updating MAXIS if the user checked the update maxis box...
IF update_MAXIS_check = 1 THEN 
...
Call write_variable_in_case_note ("~~~ Shelter Form Received on " & agency_received_date & " ~~~")
...
IF update_MAXIS_check = 1 THEN 
	...
	IF valid_addr = False AND room_and_board_check = 0 THEN CALL write_variable_in_case_note("* SHEL and HEST updated with script.")
	IF valid_addr = True AND room_and_board_check = 0 THEN CALL write_variable_in_case_note("* SHEL, HEST, and ADDR updated with script.")
	IF valid_addr = False AND room_and_board_check = 1 THEN CALL write_variable_in_case_note("* HEST updated with script.")
	IF valid_addr = True AND room_and_board_check = 1 THEN CALL write_variable_in_case_note("* HEST and ADDR updated with script.")
END IF
