	...
	Do
		Do
			If LTC_case = vbYes then		'IF LTC is selected, the Dialog is defined for an LTC case
				BeginDialog , 0, 0, 351, 435, "Verifs needed (LTC) dialog"	'Dialog name must be this for workflows'
				...
				EndDialog
				dialog			'Runs the LTC dialog'
			ElseIf LTC_case = vbNo then 			'IF LTC is NOT selected, the Dialog is defined for a standard case
				BeginDialog , 0, 0, 351, 360, "Verifs needed"		'Dialog name must be this for workflows'
				  EditBox 55, 5, 70, 15, case_number
				  EditBox 250, 5, 60, 15, verif_due_date
				 ...
				 EndDialog
				 dialog											'runs the non-ltc dialog'
			End If 
