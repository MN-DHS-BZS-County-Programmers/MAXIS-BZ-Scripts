...
If LTC_case = vbYes then		'IF LTC is selected, the Dialog is defined for an LTC case
	BeginDialog Dialog1, 0, 0, 351, 435, "Verifs needed (LTC) dialog"	'Dialog name must be this for workflows'
...
If LTC_case = vbNo then 			'IF LTC is NOT selected, the Dialog is defined for a standard case
	BeginDialog Dialog1, 0, 0, 351, 360, "Verifs needed"		'Dialog name must be this for workflows'
	  EditBox 55, 5, 70, 15, case_number
	  EditBox 250, 5, 60, 15, verif_due_date
...
DO
	Do
		Do
			dialog Dialog1 				'Calls the dialog which has been defined above
