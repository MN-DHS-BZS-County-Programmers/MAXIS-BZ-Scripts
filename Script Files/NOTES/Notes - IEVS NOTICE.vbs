...
BeginDialog IEVS_Match, 0, 0, 177, 126, "IEVS Match Received"
  ...
  DropListBox 120, 24, 40, 14, "Select"+chr(9)+"1st"+chr(9)+"2nd"+chr(9)+"3rd"+chr(9)+"4th"+chr(9)+"year", Quarter
...
BeginDialog Resolved_Non_Cooperation, 0, 0, 137, 76, "IEVS Resolved-Non Cooperation"
  DropListBox 62, 6, 50, 14, "Select"+chr(9)+"NC"+chr(9)+"CB"+chr(9)+"CC"+chr(9)+"CF"+chr(9)+"CA"+chr(9)+"CI"+chr(9)+"CP"+chr(9)+"BC"+chr(9)+"BN"+chr(9)+"BI"+chr(9)+"BP"+chr(9)+"BU"+chr(9)+"BE"+chr(9)+"BO", code
		...
		DO
			DO	
				DO				
					...
					cancel_confirmation
					'checks if an Ption has been selected
					IF OPTIONS = "Select " THEN MsgBox "You must select an option."					
				LOOP UNTIL OPTIONS <> "Select "
				'checks if a HH Memb has been entered
				IF MEMB = "" THEN MsgBox "You must enter a HHLD MEMB."
			LOOP UNTIL MEMB <> ""
			'checks if Quarter was selected.
			IF Quarter = "Select" THEN MsgBox "You must select a time period for the IEVS Match."
		LOOP UNTIL Quarter <> "Select"
		'checks if Employer was entered.
		IF Employer = "" THEN MsgBox "You must enter an employer."
	LOOP UNTIL Employer <> ""
	'checks if Address was entered.
	IF ADDR = "" THEN MsgBox "You must enter an employers address."
LOOP UNTIL ADDR <> ""
...
IF OPTIONS = "Resolved" THEN 
	DO	
		DO		
			...
			IF code = "Select" THEN MsgBox "You must select a code used."
		LOOP UNTIL code <> "Select"
		'checks if Action was completed
		IF action = "" THEN MsgBox "You must enter an action taken."
	LOOP UNTIL action <> ""
END IF
...
call write_bullet_and_variable_in_CASE_NOTE("***IEVS Notice received ", OPTIONS & "***")
...
IF code = "NC" THEN MsgBox "Add DISQ panel for this client."
